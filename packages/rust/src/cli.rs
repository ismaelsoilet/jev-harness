...
fn detect_test_command(cwd: &Path) -> String {
...
if ! TEST_OUTPUT=$(sh -c "$TEST_CMD" 2>&1); then
  printf '%s\n' "$TEST_OUTPUT" | jev-harness test-gate
  exit 1
fi
exit 0
"#,
        ...
        Commands::Init {
            ...
            if git || all {
                ...
                if git_hooks.is_dir() {
                    ...
                    let hook_script = build_git_hook(&detect_test_command(&cwd));
