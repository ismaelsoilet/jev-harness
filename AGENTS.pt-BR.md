...
> 
> *Versão do Projeto: v0.1.13 — Sincronizado nos 4 registries oficiais.*
...
4. 🛡️ **[Testes Exaustivos e Honestidade Absoluta](.agents/rules/04_testing_and_truthfulness.md)**: Postura de zero-trust, proibição de testes tautológicos, bateria de 210 testes.
...
* **Python Core (`src/jev_harness/`)**:
  ...
  - `session.py`: Telemetria de sessão, cálculo de ROI e lock atômico de concorrência (`fcntl.flock`).
* **Rust Crate (`packages/rust/`)**:
  - Implementação de alta performance em Rust estável (Tokio + Serde), provendo a biblioteca `jev_harness`, servidor MCP stdio nativo (`mcp.rs`) e os binários CLI standalone `jev` e `jev-harness`.
* **TypeScript Package (`packages/ts/`)**:
  - Pacote npm nativo `@ismaelsoilet/jev-harness` com suporte a Node.js, Bun e Deno, exportando SDK tipado, servidor MCP stdio (`mcp.ts`) e CLI executável via `npx`.
...
[Alteração de Código ou Docs]
        │
        ▼
[1. Executar 210 Testes: ./scripts/release.sh --check]
        │ (Se 100% OK)
        ▼
...
### Frescura documental (re-verificação obrigatória)
Endpoints de provedores, IDs de modelos, tiers gratuitos e preços foram verificados pela última vez em **22/09/2026**. Se passaram mais de 30 dias desde essa data, o agente DEVE re-verificá-los no [Guia Universal de Integração para Agentes](docs/AGENT_INTEGRATION_GUIDE.pt-BR.md#-acesso-a-provedores-e-chaves-de-api) antes de confiar em qualquer ID de modelo, endpoint ou preço, e DEVE registrar a data da sua própria verificação na documentação do projeto.
